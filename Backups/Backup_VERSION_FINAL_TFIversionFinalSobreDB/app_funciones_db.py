...
def db_abrir_conexion():
    try:
        ...
        print(Fore.RED + f"Error al abrir la conexión contra la DB: {e}")
        conexionDB = None
        cursorDB = None
...
def db_cerrar_conexion(conexionDB):
    try:
        ...
        print(Fore.RED + f"Falló durante commit/cierre de la DB: {e}")
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
...
def db_inicializar_tabla_productos():
    try:
        ...
        print(Fore.RED + f"Error al crear la tabla productos: {e}")
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
...
def db_obtener_productos_todos():
    ...
    try:
        ...
        print(
            Fore.RED + f"Error al intentar obtener los datos de la tabla productos: {e}"
        )
        conexionDB.close()
        lista_productos = []
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
...
def mostrar_productos_DB_comoJson():
    ...
    if not lista_productos:
        print(Fore.YELLOW + "No hay productos que mostrar")
    else:
...
def db_obtener_producto_id(id):
    ...
    try:
        ...
        print(
            Fore.RED
            + f"Error al intentar obtener los datos del producto código {id}: {e}"
        )
        conexionDB.close()
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
    return producto
...
def db_insertar_producto(codigo, nombre, descripcion, cantidad, precio, categoria):
    ...
    try:
        ...
        print(Fore.RED + f"Error insertando producto: {error}")
        conexionDB.close()
        state = False
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
    return state
...
def db_actualizar_producto(id, producto_actualizado):
    try:
        ...
        print(Fore.RED + f"Error actualizando el producto: {error}")
        conexionDB.close()
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
...
def db_eliminar_producto(id):
    try:
        ...
        print(Fore.RED + f"Error eliminando producto: {error}")
        conexionDB.close()
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
...
def db_get_productos_by_condicion(minimo_stock):
    ...
    try:
        ...
        print(Fore.RED + f"Error al obtener reporte bajo stock: {error}")
        conexionDB.close()
        lista_tuplas_productos = []
        input(Fore.GREEN + "\nPresione una tecla para continuar... ")
