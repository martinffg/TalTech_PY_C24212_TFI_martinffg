...
def solicitarCodigoProducto(contador):
    if contador >= 0:
        ...
        print(Fore.RED + "Contador inválido, debe ser mayor o igual a 0. ")
    ...
    if codigoSinValidar.isdigit():
        ...
        codigo_validado = -1  # -1 es el flag de código inválido
        print(
            Fore.RED + "El código ingresado es inválido, deben ser enteros mayores a 0"
        )
        input(Fore.GREEN + "Presione cualquier tecla para continuar... ")
...
def solicitarNombreProducto():
    nombre = ""
    while nombre == "":
        nombre = input(Fore.CYAN + "Ingresá el nombre: ")
        if nombre == "":
...
def solicitarPrecioUnitarioProducto():
    ...
    while validacion == False:
        precioStr = input(Fore.CYAN + "Ingresá el precio unitario en formato 0.0: ")
...
def solicitarCantidadProducto():
    ...
    while cantidad <= 0:
        cantidadSinValidar = input(Fore.CYAN + "Ingresá la cantidad de unidades: ")
        # Valido el ingreso con isdigit() por simplicidad, aunque bien podría haberlo manejado con exceptiones (try/except)
...
def ingresarProductoPorCodigoAlInventario(codigoIngresado, productos):
    ...
    if (
        ...
        descripcion = input(Fore.CYAN + f"Descripción: ")
        categoria = input(Fore.CYAN + f"Categoria: ")
