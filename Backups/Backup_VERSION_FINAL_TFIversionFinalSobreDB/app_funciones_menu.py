...
def mostrarProductos():
    ...
    print(
        Style.BRIGHT
        + Fore.BLUE
        + "Listado de Productos del Inventario persistidos en la DB en formato JSON"
    )
    subrayar(200)
...
def buscarProducto():
    ...
    cadenaBusqueda = input(
        Fore.CYAN + f"Ingrese código, nombre o categoría del producto buscado: "
    )
...
def swappingValoresProductoPorCampo(posicion, campo, opcionCambio):
    ...
    match campo:
        ...
        case "descripcion":
            if seCambia:
                productos[posicion]["descripcion"] = input(Fore.CYAN + "Descripción: ")
                respuesta = True
            else:
                respuesta = False
        ...
        case "categoria":
            if seCambia:
                productos[posicion]["categoria"] = input(Fore.CYAN + "Categoria: ")
                respuesta = True
            else:
                respuesta = False
            ...
            if cadenaBusqueda.isdigit():
                ...
                if len(respuestaBusquedaCodigo) > 0:
                    ...
                    while contadorCodigos < len(respuestaBusquedaCodigo):
                        posicionCodigo = respuestaBusquedaCodigo[contadorCodigos]
                        print(Fore.BLUE + f"Eliminando del inventario en memoria: ")
                        subrayar(40)
                        mostrarInfoProducto(productos[posicionCodigo])
                        productoEliminado = productos.pop(posicionCodigo)
                        separador(120)
                        print(Fore.BLUE + f"Eliminando de la DB inventario: ")
                        subrayar(40)
...
def reporteBajoStock():
    ...
    if len(productos) > 0 and db_obtener_siguiente_id_disponible() > 1:
        ...
        print(
            Style.BRIGHT
            + Fore.BLUE
            + "Se necesita definir la cantidad de productos como umbral de bajo stock."
        )
...
def mostrarOpcionesMenu():
    ...
    print(Fore.CYAN + "\t", "-" * 30)
    print(Fore.BLUE + "\t  Menú Principal del Inventario")
    print(Fore.CYAN + "\t", "-" * 30)
    print(
        Fore.GREEN
        + """ 
...
def ingresarOpcionValida(opcionStr, opcionSalida):
    ...
    if opcionStr.isdigit():
        ...
        while opcion < 1 or opcion > int(
            ...
            # print(Fore.RED +  + "error, ingrese opción válida")
            print(
                Fore.RED
                + f"Opción no válida - Ingrese un número del 1 al {opcionSalida}"
            )
            opcionStr = input(Fore.RED + f"(1-{opcionSalida}) >\t")
            if opcionStr.isdigit():
                opcion = int(opcionStr)
    else:
        opcion = (
